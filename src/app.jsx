...
import {
  ...
  Sun, Moon, Plus, Copy, ArrowRight, Camera, Check, Landmark, UserCheck, Upload, X, ChevronRight
} from 'lucide-react';
...
export default function App() {
  ...
  const [inspectingDeal, setInspectingDeal] = useState(null); // Detailed view state
  ...
  return (
    <div className={`min-h-screen ${theme.bg} font-sans transition-colors duration-200`}>
      <div className="flex h-screen overflow-hidden">
        ...
        <aside className={`w-52 ${theme.sidebar} border-r flex flex-col justify-between shrink-0`}>
          <div>
            ...
            <nav className="p-2 space-y-1">
              <button
                onClick={() => { setActiveVerifyDeal(null); setInspectingDeal(null); }}
                className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs font-semibold bg-blue-600 text-white"
              >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {/* CLIENT APP VERIFICATION PORTAL */}
            {activeVerifyDeal ? (
                ...
                <div className={`p-5 rounded-2xl border ${theme.card} space-y-4 shadow-xl`}>
                  ...
                  {verificationStep === 2 && (
                    <div className={`p-4 rounded-xl ${theme.innerCard} border ${theme.border} space-y-4`}>
                      ...
                      >
                        {isAnalyzingBiometrics ? 'Executing Amazon Rekognition CompareFaces...' : 'Verify Identity & Facial Match'}
                      </button>
                    </div>
                  )}
                  ...
                  {verificationStep === 3 && (
                    <div className={`p-4 rounded-xl ${theme.innerCard} border ${theme.border} space-y-4`}>
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-bold text-white">Step 3: Draw Digital Signature</h4>
                      </div>
                ...
                {filteredDeals.map((deal) => (
                  <div key={deal.id} className={`p-4 rounded-xl border ${theme.card} space-y-3 shadow-md`}>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                      ...
                      >
                        <Eye className="w-3 h-3 text-blue-400" /> View Audit File
                      </button>
      ...
      {/* INSPECT DEAL VERIFICATION AUDIT MODAL */}
      {inspectingDeal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className={`${theme.card} border rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5 space-y-4 shadow-2xl`}>
            <div className="flex justify-between items-center border-b pb-3 border-slate-800">
              <div>
                <span className="text-[10px] font-bold text-blue-500 font-mono">VERIFICATION AUDIT FILE</span>
            ...
            {/* 1. EMPLOYER & BANK DEPOSIT STREAM */}
            <div className={`p-3.5 rounded-xl ${theme.innerCard} border ${theme.border} space-y-2`}>
              <div className="flex items-center justify-between border-b pb-2 border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-white">Employer & Income Deposits</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                <div>
                  <span className="text-slate-500 block text-[9px]">Verified Employer</span>
                  <span className="font-semibold text-slate-200">{inspectingDeal.employerDetails?.name || 'Acme Corp'}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[9px]">Monthly Net Deposits</span>
                  <span className="font-bold text-emerald-400">${inspectingDeal.employerDetails?.monthlyNetDeposit?.toLocaleString() || '8,450'} / mo</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[9px]">Pay Frequency</span>
                  <span className="text-slate-300">{inspectingDeal.employerDetails?.payFrequency || 'Bi-Weekly'}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[9px]">AWS Textract Confidence</span>
                  <span className="text-blue-400 font-mono">{inspectingDeal.employerDetails?.confidence || '99.4%'}</span>
                </div>
              </div>
            </div>

            {/* 2. DRIVER LICENSE VS SELFIE MATCH */}
            <div className={`p-3.5 rounded-xl ${theme.innerCard} border ${theme.border} space-y-2`}>
              <div className="flex items-center justify-between border-b pb-2 border-slate-800">
                <div className="flex items-center gap-2">
                  <Camera className="w-4 h-4 text-purple-400" />
                  <span className="text-xs font-bold text-white">Biometric ID & Selfie Verification</span>
                </div>
                <span className="text-[10px] font-mono bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded font-bold">
                  Match: {inspectingDeal.verifications.id.score}%
                </span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-center pt-1">
                <div className="bg-slate-900 p-2 rounded-lg border border-slate-800 flex flex-col items-center">
                  <div className="w-16 h-12 bg-slate-800 rounded border border-slate-700 flex items-center justify-center mb-1 text-[9px] text-slate-400 font-mono">
                    CANADIAN ID
                  </div>
                  <span className="text-[9px] text-slate-400">Driver License Photo</span>
                </div>
                <div className="bg-slate-900 p-2 rounded-lg border border-slate-800 flex flex-col items-center">
                  <div className="w-12 h-12 rounded-full bg-slate-800 border border-purple-500/40 flex items-center justify-center mb-1 text-[9px] text-purple-300 font-mono">
                    3D SELFIE
                  </div>
                  <span className="text-[9px] text-slate-400">Live Liveness Match</span>
                </div>
              </div>
            ...
            <div className={`p-3.5 rounded-xl ${theme.innerCard} border ${theme.border} space-y-2`}>
              <div className="flex items-center justify-between border-b pb-2 border-slate-800">
                ...
                <span className="text-[10px] font-mono bg-indigo-500/10 text-indigo-400 px-2 py-0.5 rounded font-bold">
                  Match: {inspectingDeal.verifications.signature.score}%
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-center pt-1">
                <div className="bg-slate-900 p-2 rounded-lg border border-slate-800">
                  ...
                  <span className="text-[9px] text-slate-400 block border-t border-slate-800 pt-1">License Signature</span>
                </div>
                <div className="bg-slate-900 p-2 rounded-lg border border-slate-800">
                  <div className="h-8 flex items-center justify-center text-emerald-400 font-serif italic text-xs">
                    S. Jenkins
                  </div>
                  <span className="text-[9px] text-slate-400 block border-t border-slate-800 pt-1">Drawn PKI Canvas</span>
                </div>
              </div>
            </div>
